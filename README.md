...
`build-profile.json5` 引用本机 DevEco 调试签名材料（`~/.ohos/config`，bundleName 与 profile 绑定）。
当前为过渡配置（`com.richerfu.h_openconnect`）；正式 bundleName `com.richerfu.native_binding_demo`
需在 DevEco Studio 中对工程执行一次 auto-sign 生成材料后替换。
